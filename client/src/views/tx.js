...
const txHeader = (tx, { tipHeight, mempool, feeEst, t
  ...
  <div className="stats-table font-p2">
    ...
    { feerate != null && <div>
      ...
      <div>
        <span className="amount">{t`${formatSat(tx.fee)} (${feerate.toFixed(1)} nano eTHB)`}</span>
        { overpaying > OVERPAYMENT_WARN &&
          <p className={`text-${ overpaying > OVERPAYMENT_WARN*1.5 ? 'danger' : 'warning' } mb-0`} title={t`compared to bitcoind's suggested fee of ${feeEst[2].toFixed(1)} nano eTHB for confirmation within 2 blocks`}>
